...
RSpec.describe Location do
  ...
  context 'instance methods' do
    let!(:actual_methods) { %i(id state email country street city zip_code phone_number ) }
    let!(:expected_methods) do
      Location.public_instance_methods(false).select! { |method| method unless method.to_s.end_with? '=' }
    end
